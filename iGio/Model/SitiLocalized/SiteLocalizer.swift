...
class SiteLocalizer {
    ...
    func getLocations(of type: LocationType, saveRecords: Bool = true, completion: (([LocationCodable]) -> Void)? = nil) {
        var request = BasicRequest(hostname: AppURL.hostname, requestType: RequestType.locations.rawValue)
    ...
    public func fetchLocalizedWebsites(for location: LocationCodable? = nil, saveRecords: Bool = true, completion: @escaping (Result<LocalizedList, Error>) -> Void) {
        var req = BasicRequest(hostname: AppURL.hostname, requestType: RequestType.localizedSites.rawValue)
        
        if let locationID = location?.id {
            req = BasicRequest(hostname: AppURL.hostname, requestType: RequestType.localizedSites.rawValue, args: ["locationID" : "\(locationID)"])
        }
    ...
    public func fetchPreghiereSites(saveRecords: Bool = true, completion: @escaping (Result<LocalizedList, Error>) -> Void) {
        ...
        let req = BasicRequest(hostname: AppURL.hostname, requestType: RequestType.preghiere.rawValue)
